...
def get_data(geojson_path, output_path2, data):
    ...
    with rasterio.open(raster_path) as raster:
        ...
        for index, sector in sectors.iterrows():
            ...
            white_pixel_count = np.sum(masked_image == 255)
        ...
        with open('data.csv', 'w', newline='') as file:
            ...
            writer.writerow(["Sector ID", "Total Pixel Count", "White Pixel Count", "Mean Pixel Value"])
            for key, value in data.items():
                writer.writerow([key, value['total_pixel_count'], value['white_pixel_count'], value['mean_pixel_value']])
